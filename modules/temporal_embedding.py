...
class GraphEmbedding(nn.Module):
  def __init__(self, node_features, edge_features, memory, neighbor_finder, time_encoder, n_layers,
    ...
    self.node_memory_transform = nn.Linear(node_features.shape[1], node_features.shape[1])
  ...
  def compute_embedding(self, memory, source_nodes, timestamps, n_layers, n_neighbors=20):
    ...
    if self.use_memory:
      source_node_features = self.node_memory_transform(memory[source_nodes, :]) + source_node_features

    if n_layers == 0:
      return source_node_features
    else:
      source_node_embedding = self.compute_embedding(memory, source_nodes, timestamps, n_layers=n_layers - 1,
      ...
      source_embedding = self.aggregate(n_layers, source_node_embedding,
  ...
  def aggregate(self, n_layers, source_node_features, source_nodes_time_embedding,
                neighbor_embeddings,
                edge_time_embeddings, edge_features):
    return None
