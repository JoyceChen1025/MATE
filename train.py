...
for i in range(args.n_runs):
  ...
  test_ap, test_auc, test_acc = eval_edge_prediction(model=mate,
                                                              ...
                                                              data=test_data,
                                                              n_neighbors=NUM_NEIGHBORS)
  n_runs_auc.append(test_auc)
  n_runs_ap.append(test_ap)
  n_runs_acc.append(test_acc)
...
print(n_runs_acc)
logger.info(
      'Test statistics: -- auc: {:.2f}({:.2f}), ap: {:.2f}({:.2f}), acc: {:.2f}({:.2f})'.format(np.mean(n_runs_auc)*100, np.std(n_runs_auc)*100, np.mean(n_runs_ap)*100, np.std(n_runs_ap)*100, np.mean(n_runs_acc)*100, np.std(n_runs_acc)*100))
